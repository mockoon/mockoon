 ...
 *
 */
export class NewOpenAPIConverter {
  /**
  ...
  private dereference(spec: any): any {
    ...
    const resolveRef = (ref: string): any => {
      ...
      for (const segment of refPath) {
        if (current[segment] === undefined) {
          throw new Error(`Reference ${ref} not found in the specification`);
        }
    ...
    const dereferenceObject = function (obj: any): any {
      if (Array.isArray(obj)) {
        return obj.map(dereferenceObject);
        ...
        if (obj.$ref) {
          ...
          // Mark the resolved object as seen
          const placeholder: any = {};
          seen.set(resolved, placeholder);

          // Dereference the resolved object and update the placeholder
          const dereferenced = dereferenceObject(resolved);
        ...
        for (const key of Object.keys(obj)) {
          newObj[key] = dereferenceObject(obj[key]);
        }
