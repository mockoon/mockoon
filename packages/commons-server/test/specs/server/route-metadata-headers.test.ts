...
describe('Route metadata headers', () => {
  ...
  it('should set route response metadata headers when enabled', async () => {
...
describe('Route metadata headers disabled by default', () => {
  let environment: Environment;
  let server: MockoonServer;
  ...
  it('should not set route metadata headers unless enabled', async () => {
    const response = await fetch('http://localhost:3012/test');

    strictEqual(response.status, 200);
    strictEqual(response.headers.get('x-mockoon-route-uuid'), null);
    strictEqual(response.headers.get('x-mockoon-route-response-no'), null);
    strictEqual(response.headers.get('x-mockoon-route-response-uuid'), null);
  });
});
