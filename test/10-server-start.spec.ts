...
describe('Environment start/stop/restart', () => {
  ...
  it('Start default selected environment', async () => {
    ...
    await tests.spectron.client.waitForExist('.menu-columns:first-of-type .menu-list .nav-item .nav-link.running', 10000);
  });

  it('Stop default selected environment', async () => {
    await tests.spectron.client.element('.btn i[ngbtooltip="Stop server"]').click();
    await tests.spectron.client.waitForExist('.menu-columns:first-of-type .menu-list .nav-item .nav-link:not(.running)', 10000);
  });
});
